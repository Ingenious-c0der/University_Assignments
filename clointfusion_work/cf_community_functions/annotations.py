from clointfusion.functions import browser
from clointfusion.functions import folder
from clointfusion.functions import keyboard
from clointfusion.functions import message
from clointfusion.functions import mouse
from clointfusion.functions import screen_srcaping
from clointfusion.functions import string
from clointfusion.functions import utility
from clointfusion.functions import window
from clointfusion.functions import voice
...
def message_counter_down_timer(strMsg="Welcome Clointfusion in (seconds)", start_value=5):
    ...
    """
    return message.message_counter_down_timer(strMsg, start_value)
...
def message_pop_up(strMsg="Welcome to Clointfusion.", delay=3):
    ...
    Returns: [status]
    """
    message.message_pop_up(strMsg="Welcome to Clointfusion.", delay=3)
...
def message_flash(msg="Welcome to Clointfusion.", delay=3):
    ...
    Returns: [status]
    """
    message.message_flash(msg, delay)


def message_toast(msg="Welcome to Clointfusion.", website_url="", file_folder_path=""):
    """
    ...
    Returns: [status]
    """
    message.message_toast(msg, website_url, file_folder_path)
...
def mouse_click(x='5', y='5', left_or_right="left", no_of_clicks=1):
    ...
    """
    return mouse.mouse_click(x, y, left_or_right, no_of_clicks)
...
def mouse_drag_from_to(x1="5", y1="5", x2="10", y2="10", delay=0.5):
    ...
    Returns: [status]
        bool: Whether the function is successful or failed.
    """
    return mouse.mouse_drag_from_to(x1, y1, x2, y2, delay)
...
def text_to_speech(audio, show=True, rate=170)->None:
    ...
    """
    return voice.text_to_speech(audio, show, rate)
